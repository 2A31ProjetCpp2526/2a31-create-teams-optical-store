...
#include <QFileInfo>
#include <QDateTime>
// Arduino support comes through opticstor.h - NO need to include here!
    ...
    ui(new Ui::opticstor)
{
    ...
    // ARDUINO TEST BUTTON CONNECTION
    ...
    // Employés connections - ADD THESE
    connect(ui->ajouter_3, &QPushButton::clicked, this, &opticstor::on_ajouter_3_clicked);
    connect(ui->afficher_3, &QPushButton::clicked, this, &opticstor::on_afficher_3_clicked);
    connect(ui->supprimer_3, &QPushButton::clicked, this, &opticstor::on_supprimer_3_clicked);
    connect(ui->modifier_3, &QPushButton::clicked, this, &opticstor::on_modifier_3_clicked);
    connect(ui->rechercher_2, &QPushButton::clicked, this, &opticstor::on_rechercher_2_clicked);
    connect(ui->sendchat_2, &QPushButton::clicked, this, &opticstor::sendMessage);
    connect(ui->trie_2, &QPushButton::clicked, this, &opticstor::on_trie_2_clicked);
    connect(ui->statistique_2, &QPushButton::clicked, this, &opticstor::on_statistique_2_clicked);
    connect(ui->pdf_2, &QPushButton::clicked, this, &opticstor::on_pdf_2_clicked);
    connect(ui->btn_checkin_2, &QPushButton::clicked, this, &opticstor::on_btn_checkin_clicked);
    connect(ui->btn_checkout_2, &QPushButton::clicked, this, &opticstor::on_btn_checkout_clicked);
    connect(ui->sendchat_2, &QPushButton::clicked, this, &opticstor::sendMessage);
    ...
    connect(ui->btnPerformance_2, &QPushButton::clicked, this, [=](){
...
void opticstor::updatecommande(){
    ...
    if(ui->idclientc->currentIndex()!=0){
        ...
        query.exec();
        QString message ="updated"; // QString("UPDATE:%1\n").arg(actualId);
...
void opticstor::supprimcommande(){
    ...
    if(deleteContenir.exec()){
        ...
        if(query.exec()){
            ...
            QString message ="deleted"; // QString("UPDATE:%1\n").arg(actualId);
            A.write_to_arduino(message.toUtf8());
            affichecommande();
        } else {
...
void opticstor::loadtypesmarquesComboBox() {
    ...
    while(query.next()) {
        ...
        ui->marquec->addItem(query.value(0).toString());
    }
...
void opticstor::autrecmd(){
    ...
    if (query.exec() && query.next()) {
        ...
        if(queryy.exec()){
            ...
            QString message ="added"; // QString("UPDATE:%1\n").arg(actualId);
            A.write_to_arduino(message.toUtf8());
        }else {
...
void opticstor::ajoutcommande(){
    ...
    if (querry.exec() && querry.next()) {
        ...
        if (query.exec() && squery.exec()) {
            ...
            QString message ="affected"; // QString("UPDATE:%1\n").arg(actualId);
            A.write_to_arduino(message.toUtf8());
            affichecommande();
        } else {
...
void opticstor::on_ajouter_3_clicked()
{
...
void opticstor::on_supprimer_3_clicked()
{
    int id = ui->id_supp_2->text().toInt();  // take the ID from the input
    ...
    if (e.supprimer(id)) {
        ...
        ui->id_supp_2->clear();     // clear input after deleting
    } else {
...
// =====================================
void opticstor::afficherTableemployes()
{
    employe empTmp;
...
void opticstor::on_modifier_3_clicked()
{
    int id = ui->id_mod_2->text().toInt();
    QString nom = ui->nom_mod_2->text().trimmed();
    QString prenom = ui->prenom_mod_2->text().trimmed();
    QString type = ui->type_mod_2->text().trimmed();
    QDate dateEmb = ui->date_mod_2->date();
...
// === Bouton Afficher =================
// =====================================
void opticstor::on_afficher_3_clicked()
{
    afficherTableemployes();
}
void opticstor::on_rechercher_2_clicked()
{
    QString idText = ui->id_rech_2->text().trimmed();
...
void opticstor::on_trie_2_clicked()
{
...
void opticstor::on_statistique_2_clicked()
{
    ...
    ui->tableWidget_s_2->setRowCount(1);
    ui->tableWidget_s_2->setColumnCount(1);
    ui->tableWidget_s_2->setCellWidget(0, 0, chartView);
}
...
void opticstor::sendMessage() {
    QString userMsg = ui->chat_2->text().trimmed();
    if (userMsg.isEmpty()) return;

    ui->boxchat_2->append("You: " + userMsg);
    ui->chat_2->clear();
    ...
    QString apiKey = "AIzaSyC4YwLFwxSbJSaSx4IPP9nTydetiYmvkoQ";
    ...
    connect(&manager, &QNetworkAccessManager::finished, this, &opticstor::onResponse);
    manager.post(request, QJsonDocument(json).toJson());
}

void opticstor::onResponse(QNetworkReply *reply) {
    if (reply->error()) {
        ui->boxchat_2->append("Error: " + reply->errorString());
        reply->deleteLater();
        return;
    }
    ...
    if (!choices.isEmpty()) {
        ...
        ui->boxchat_2->append("AI: " + answer);
    } else {
        ui->boxchat_2->append("AI: (no response)");
    }
...
void opticstor::on_pdf_2_clicked()
{
...
// --- Check-in ---
void opticstor::on_btn_checkin_clicked()
{
    int id = ui->lineEdit_id_emp_2->text().toInt();
    if (id <= 0) {
        QMessageBox::warning(this, "Erreur", "Veuillez entrer un ID valide !");
        return;
    }
...
// --- Check-out ---
void opticstor::on_btn_checkout_clicked()
{
    int id = ui->lineEdit_id_emp_2->text().toInt();
    if (id <= 0) {
        QMessageBox::warning(this, "Erreur", "Veuillez entrer un ID valide !");
...
void opticstor::afficherTablePresence()
{
    ...
    ui->tableView_presence_2->clearContents();
    ui->tableView_presence_2->setRowCount(0);

    // 3. Updated Headers and Column Count (4 columns instead of 5)
    QStringList headers = {"ID Employé", "Date", "Heure Entrée", "Heures Travail"};
    ui->tableView_presence_2->setColumnCount(headers.size()); // Size is now 4
    ui->tableView_presence_2->setHorizontalHeaderLabels(headers);

    int row = 0;
    while (query.next()) {
        ui->tableView_presence_2->insertRow(row);
        ...
        // Column 0: ID Employé
        ui->tableView_presence_2->setItem(row, 0, new QTableWidgetItem(query.value("id_employe").toString()));

        // Column 1: Date
        ui->tableView_presence_2->setItem(row, 1, new QTableWidgetItem(query.value("date_jour").toDate().toString("yyyy-MM-dd")));

        // Column 2: Heure Entrée
        ui->tableView_presence_2->setItem(row, 2, new QTableWidgetItem(hEntree.isNull() ? "" : hEntree.toTime().toString("HH:mm:ss")));

        // Column 3: Heures Travail (Moved from index 4)
        ui->tableView_presence_2->setItem(row, 3, new QTableWidgetItem(hTravail.isNull() ? "" : QString::number(hTravail.toDouble(), 'f', 2)));

        row++;
    }

    ui->tableView_presence_2->resizeColumnsToContents();
}
void opticstor::afficherPerformancePresence(int totalWorkingDays)
{
    QSqlQuery query;
    query.prepare(R"(
    ...
    ui->tableWidget_performance_2->clear();
    QStringList headers = {"ID", "Nom", "Prénom", "Jours Présent", "Total Jours", "Score (%)"};
    ui->tableWidget_performance_2->setColumnCount(headers.size());
    ui->tableWidget_performance_2->setHorizontalHeaderLabels(headers);
    ui->tableWidget_performance_2->setRowCount(0);
    ...
    while(query.next()) {
        ...
        ui->tableWidget_performance_2->insertRow(row);
        ui->tableWidget_performance_2->setItem(row, 0, new QTableWidgetItem(query.value("id").toString()));
        ui->tableWidget_performance_2->setItem(row, 1, new QTableWidgetItem(query.value("nom").toString()));
        ui->tableWidget_performance_2->setItem(row, 2, new QTableWidgetItem(query.value("prenom").toString()));
        ui->tableWidget_performance_2->setItem(row, 3, new QTableWidgetItem(QString::number(jours_present)));
        ui->tableWidget_performance_2->setItem(row, 4, new QTableWidgetItem(QString::number(totalWorkingDays)));
        ui->tableWidget_performance_2->setItem(row, 5, new QTableWidgetItem(QString::number(score, 'f', 2) + "%"));
        row++;
    }

    ui->tableWidget_performance_2->resizeColumnsToContents();
}
