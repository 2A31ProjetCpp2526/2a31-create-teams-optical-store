...
{
    // Validation des données (company name and email required)
    if (NOM.isEmpty() || EMAIL.isEmpty()) {
        QMessageBox::warning(nullptr, QObject::tr("Validation"),
                           QObject::tr("Nom d'entreprise et email sont obligatoires !"));
    ...
    QSqlQuery query(db);
    // If an ID was provided, include it in the INSERT (some DB schemas require explicit ID)
    if (ID_FOURNISSEUR > 0) {
        ...
        // Use singular table and columns matching desired schema
...
QSqlQueryModel* Fournisseur::afficher()
{
    ...
    QSqlQuery query(db);
    // Select using the schema requested by the user
...
QSqlQueryModel* Fournisseur::afficherTrie(const QString& orderBy)
{
    QSqlQueryModel* model = new QSqlQueryModel();
    QSqlDatabase db = QSqlDatabase::database();

    if (!db.isOpen()) {
        qDebug() << "❌ Erreur: BD non connectée pour affichage tri";
        return model;
    }

    QString order = orderBy.trimmed();
    if (order.isEmpty()) order = "ID_FOURNISSEUR DESC";

    QSqlQuery query(db);
    QString q = QString("SELECT ID_FOURNISSEUR, NOM_ENTREPRISE, NUM_TELEPHONE, EMAIL FROM FOURNISSEUR ORDER BY %1").arg(order);

    if (!query.exec(q)) {
        qDebug() << "❌ Erreur SQL (afficherTrie):" << query.lastError().text();
        return model;
    }
...
bool Fournisseur::modifier(int id)
{
    ...
    // Validation: only require company name, telephone and email for the simplified schema
    if (NOM.isEmpty() || TELEPHONE.isEmpty() || EMAIL.isEmpty()) {
        QMessageBox::warning(nullptr, QObject::tr("Validation"),
                           QObject::tr("Nom d'entreprise, téléphone et email sont obligatoires !"));
        return false;
    }
    ...
    QSqlQuery query(db);
    // Update according to the simplified fournisseur schema
...
QSqlQueryModel* Fournisseur::rechercher(const QString& mot_cle, const QString& colonne)
{
    ...
    // Support searching by ID or company name
    QString colonneName = (colonne == "ID") ? "ID_FOURNISSEUR" : "NOM_ENTREPRISE";
