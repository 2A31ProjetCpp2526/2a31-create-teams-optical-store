    ...
    , ui(new Ui::opticstor)
{
   ...
   // connect(ui->actualiser_produit, &QPushButton::clicked, this, &opticstor::on_actualiser_produit_clicked);
    ...
    // ==================== CONFIGURATION POUR LA GESTION DES FOURNISSEURS ====================
    
    // Initialiser la table des fournisseurs - Charger les données
    ...
    ui->lineEdit_51->setValidator(new QIntValidator(0, 99999999, this));
    ui->lineEdit_46->setValidator(new QIntValidator(0, 999999, this));
...
{
    QString prenom = ui->lineEdit_47->text();
    QString nom = ui->lineEdit_50->text();
    QString telephone = ui->lineEdit_51->text();
    QString email = ui->lineEdit_53->text();
    QString type = ui->comboBox_16->currentText();

    // Validation des champs obligatoires
    if (nom.isEmpty()) {
        QMessageBox::warning(this, QObject::tr("Erreur - Champ manquant"),
                             QObject::tr("❌ Le champ <b>Nom</b> du fournisseur est vide!"), QMessageBox::Ok);
    ...
    if (prenom.isEmpty()) {
        QMessageBox::warning(this, QObject::tr("Erreur - Champ manquant"),
                             QObject::tr("❌ Le champ <b>Prénom</b> du fournisseur est vide!"), QMessageBox::Ok);
    ...
    if (telephone.isEmpty()) {
        QMessageBox::warning(this, QObject::tr("Erreur - Champ manquant"),
                             QObject::tr("❌ Le champ <b>Téléphone</b> du fournisseur est vide!"), QMessageBox::Ok);
    ...
    if (email.isEmpty()) {
        QMessageBox::warning(this, QObject::tr("Erreur - Champ manquant"),
                             QObject::tr("❌ Le champ <b>Email</b> du fournisseur est vide!"), QMessageBox::Ok);
        ui->lineEdit_53->setFocus();
        return;
    }

    Fournisseur f(nom, prenom, telephone, email, type);
    bool test = f.ajouter();

    if (test) {
        QMessageBox::information(this, QObject::tr("✅ Succès"),
                                 QObject::tr("Fournisseur <b>ajouté</b> avec succès!\n\n"
                                             "Nom: <b>%1</b>\n"
                                             "Prénom: <b>%2</b>\n"
                                             "Téléphone: <b>%3</b>\n"
                                             "Email: <b>%4</b>").arg(nom).arg(prenom).arg(telephone).arg(email), QMessageBox::Ok);
        ...
        QMessageBox::critical(this, QObject::tr("❌ Erreur d'ajout"),
                              QObject::tr("Échec de l'ajout du fournisseur!\n\n"
                                          "Raisons possibles:\n"
                                          "• Les données sont invalides\n"
                                          "• Problème de connexion à la base de données"), QMessageBox::Ok);
    }
}

void opticstor::on_supprimer_fournisseur_clicked()
{
    QString idText = ui->lineEdit_46->text();

    if (idText.isEmpty()) {
        QMessageBox::warning(this, QObject::tr("Erreur - Champ manquant"),
                             QObject::tr("❌ Le champ <b>ID à supprimer</b> est vide!\n\n"
                                         "Veuillez entrer l'ID du fournisseur à supprimer."), QMessageBox::Ok);
    ...
    QMessageBox::StandardButton reply;
    reply = QMessageBox::question(this, QObject::tr("Confirmation de suppression"),
                                  QObject::tr("⚠️ Voulez-vous vraiment supprimer le fournisseur avec l'ID <b>%1</b>?\n\n"
                                              "Cette action est irréversible!").arg(id),
                                  QMessageBox::Yes | QMessageBox::No);
    ...
    if (reply == QMessageBox::Yes) {
        if (tmpFournisseur.supprimer(id)) {
            QMessageBox::information(this, QObject::tr("✅ Succès"),
                                     QObject::tr("Fournisseur avec l'ID <b>%1</b> supprimé avec succès!").arg(id), QMessageBox::Ok);
            chargerTableFournisseurs();
            ui->lineEdit_46->clear();
        } else {
            QMessageBox::critical(this, QObject::tr("❌ Erreur de suppression"),
                                  QObject::tr("Échec de la suppression du fournisseur avec l'ID <b>%1</b>!").arg(id), QMessageBox::Ok);
        }
    }
}
...
void opticstor::on_modifier_fournisseur_clicked()
{
    ...
    if (idText.isEmpty()) {
        QMessageBox::warning(this, QObject::tr("Erreur - Champ manquant"),
                             QObject::tr("❌ Le champ <b>ID</b> est vide!\n\n"
                                         "Veuillez d'abord sélectionner un fournisseur à modifier."), QMessageBox::Ok);
        ui->lineEdit_46->setFocus();
        return;
    }
    ...
    if (id <= 0) {
        QMessageBox::warning(this, QObject::tr("Erreur - ID invalide"),
                             QObject::tr("❌ L'<b>ID</b> est invalide!"), QMessageBox::Ok);
        ui->lineEdit_46->setFocus();
        return;
    }

    if (nom.isEmpty() || prenom.isEmpty() || telephone.isEmpty() || email.isEmpty()) {
        QMessageBox::warning(this, QObject::tr("Erreur - Champs manquants"),
                             QObject::tr("❌ Tous les champs doivent être remplis!"), QMessageBox::Ok);
        return;
    }

    tmpFournisseur.setNom(nom);
    tmpFournisseur.setPrenom(prenom);
    tmpFournisseur.setTelephone(telephone);
    tmpFournisseur.setEmail(email);
    tmpFournisseur.setType(type);

    if (tmpFournisseur.modifier(id)) {
        QMessageBox::information(this, QObject::tr("✅ Succès"),
                                 QObject::tr("Fournisseur avec l'ID <b>%1</b> modifié avec succès!").arg(id), QMessageBox::Ok);
        chargerTableFournisseurs();
        ui->lineEdit_46->clear();
        ui->lineEdit_47->clear();
        ui->lineEdit_50->clear();
        ...
        QMessageBox::critical(this, QObject::tr("❌ Erreur de modification"),
                              QObject::tr("Échec de la modification du fournisseur avec l'ID <b>%1</b>!").arg(id), QMessageBox::Ok);
    }
}

void opticstor::on_rechercher_fournisseur_clicked()
{
    QString motCle = ui->lineEdit_45->text();
    QString colonne = ui->comboBox_13->currentText();

    if (motCle.isEmpty()) {
        QMessageBox::warning(this, QObject::tr("Erreur - Champ manquant"),
                             QObject::tr("❌ Le mot clé de recherche est vide!"), QMessageBox::Ok);
        ui->lineEdit_45->setFocus();
        return;
    }

    QSqlQueryModel* model = tmpFournisseur.rechercher(motCle, colonne);
    
    if (model->rowCount() == 0) {
        QMessageBox::warning(this, QObject::tr("⚠️ Aucun résultat"),
                            QObject::tr("Aucun fournisseur trouvé pour: <b>%1</b>").arg(motCle), QMessageBox::Ok);
    } else {
        QMessageBox::information(this, QObject::tr("✅ Recherche effectuée"),
                                QObject::tr("Résultats trouvés: <b>%1</b>").arg(model->rowCount()), QMessageBox::Ok);
    }
}
...
void opticstor::on_actualiser_fournisseur_clicked()
{
    ...
    QMessageBox::information(this, QObject::tr("✅ Actualisé"),
                            QObject::tr("La liste des fournisseurs a été actualisée!"), QMessageBox::Ok);
}
...
void opticstor::chargerTableFournisseurs()
{
    ...
    ui->tableWidget_22->setStyleSheet(styleSheet);
...
void opticstor::on_ajouter_employe_clicked()
{
    ...
    QDate date_emb = ui->date_emb->date();

    employe e(nom, prenom, salaire, date_emb, type);

    if (e.ajouter()) {
        QMessageBox::information(this, "Succès", "Employé ajouté avec succès !");
        chargerTableEmployes(); // refresh after adding
    } else {
        QMessageBox::critical(this, "Erreur", "Échec de l'ajout !");
    }
}

void opticstor::chargerTableEmployes()
{
    employe empTmp;
    QSqlQueryModel* model = empTmp.afficher();

    if (!model) {
        QMessageBox::warning(this, "Erreur", "Impossible de charger la table des employés !");
        return;
    }

    QTableWidget* table = ui->tableView_employes; // your table widget
    table->setRowCount(model->rowCount());
    table->setColumnCount(model->columnCount());
    table->setHorizontalHeaderLabels({"ID", "Nom", "Prénom", "Date Embauche", "Salaire", "Type"});

    for (int row = 0; row < model->rowCount(); ++row) {
        for (int col = 0; col < model->columnCount(); ++col) {
            table->setItem(row, col, new QTableWidgetItem(model->data(model->index(row, col)).toString()));
        }
    }
...
void opticstor::on_supprimer_employe_clicked()
{
    ...
    if (e.supprimer(id)) {
        ...
        chargerTableEmployes();  // refresh tableView if you display employees
...
void opticstor::on_modifier_employe_clicked()
{
    ...
    if (e.modifier()) {
        ...
        chargerTableEmployes();  // Refresh the table after update
    } else {
